...
def seasonals_chart(tick):
	...
	for attempt in range(3):  # Try up to 3 times
	    try:
	        spx_rank = yf.Ticker(ticker).history(start=start_date, end=end_date, auto_adjust=True)
	...
	days = yf.download(ticker, start="2023-12-31", end=now,progress=False)
...
megas_list=['^GSPC','SPY','^DJI','^RUT','^NDX','QQQ','^SOX','^IXIC','^RUO','^GDAXI','^FTSE','^HSI','^N225','TLT','^VIX']
