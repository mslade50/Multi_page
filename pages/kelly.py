...
def kelly_sizing_page():
    ...
    # Optional cost input for option contracts
    cost = st.number_input("Option Cost (optional)", min_value=0.0, step=0.01)
    ...
    if st.button("Calculate Bet Size"):
        if payoff > 0 and win_prob > 0 and win_prob <= 1 and fractional_kelly > 0 and account_size > 0:
            ...
            if cost > 0:
                num_contracts = dollar_bet // (cost * 100)
                st.success(f"Recommended Number of Contracts: {int(num_contracts)}")
        else:
